...
def rvi(df: pd.DataFrame, parameters: dict = None, columns: dict = None) -> tuple:
    """
    Calculates the Relative Vigor Index (RVI), a technical indicator that measures the conviction 
    ...
    Returns:
        tuple: A tuple containing the RVI DataFrame (RVI, Signal) and a list of column names.
    ...
    - Crossovers: RVI crossing above the Signal Line is a buy signal; below is a sell signal.
    ...
    # RVI Numerator: (Close - Open) + 2*(C1-O1) + 2*(C2-O2) + (C3-O3) / 6
    co = c - o
    ...
    rvi_val = sma_num / sma_denom

    # Signal Line: (RVI + 2*RVI1 + 2*RVI2 + RVI3) / 6
    signal = (rvi_val + 2 * rvi_val.shift(1) + 2 * rvi_val.shift(2) + rvi_val.shift(3)) / 6

    rvi_col = f'RVI_{window}'
    sig_col = 'RVI_SIG'
